...
cat > /tmp/server_large_ext.cnf << 'EOF'
[ext]
keyUsage = critical,digitalSignature,keyEncipherment
extendedKeyUsage = serverAuth
EOF
openssl x509 -req -days 3650 -in server_large.csr -CA ca_large.crt -CAkey ca_large.key \
    -CAcreateserial -extfile /tmp/server_large_ext.cnf -extensions ext -out server_large.crt 2>/dev/null
...
cat > /tmp/client_large_ext.cnf << 'EOF'
[ext]
keyUsage = critical,digitalSignature
extendedKeyUsage = clientAuth
EOF
openssl x509 -req -days 3650 -in client_large.csr -CA ca_large.crt -CAkey ca_large.key \
    -CAcreateserial -extfile /tmp/client_large_ext.cnf -extensions ext -out client_large.crt 2>/dev/null
