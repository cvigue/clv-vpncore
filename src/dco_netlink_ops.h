...
{
    if (response.size() < sizeof(struct nlmsghdr))
    {
        logger.error("DCO: {} failed: truncated netlink response ({} bytes)",
                     op_name,
                     response.size());
        return false;
    }
    ...
    int msg_len = static_cast<int>(response.size());
    if (!NLMSG_OK(nlh, msg_len))
    {
        logger.error("DCO: {} failed: malformed netlink header", op_name);
        return false;
    }
    ...
    if (nlh->nlmsg_type == NLMSG_ERROR)
    {
        ...
        {
            logger.error("DCO: {} failed: malformed NLMSG_ERROR payload", op_name);
            return false;
        }
                             ...
                             spdlog::logger &logger)
{
    ...
    if (!NlaPut(buf, offset, kAttrsCap, OVPN_NEW_KEY_ATTR_PEER_ID, &peer_id, sizeof(peer_id)) || !NlaPut(buf, offset, kAttrsCap, OVPN_NEW_KEY_ATTR_KEY_SLOT, &key_slot, sizeof(key_slot)) || !NlaPut(buf, offset, kAttrsCap, OVPN_NEW_KEY_ATTR_KEY_ID, &key_id, sizeof(key_id)) || !NlaPut(buf, offset, kAttrsCap, OVPN_NEW_KEY_ATTR_CIPHER_ALG, &dco_cipher_alg, sizeof(dco_cipher_alg)))
    {
        logger.error("DCO: Buffer overflow in OVPN_CMD_NEW_KEY key attrs");
        return false;
    }
    ...
    // ENCRYPT_KEY (nested)
    {
        ...
        if (!ek)
            return false;
        if (!NlaPut(buf, offset, kAttrsCap, OVPN_KEY_DIR_ATTR_CIPHER_KEY, encrypt_key, cipher_key_size) || !NlaPut(buf, offset, kAttrsCap, OVPN_KEY_DIR_ATTR_NONCE_TAIL, encrypt_nonce_tail, 8))
        {
            logger.error("DCO: Buffer overflow in encrypt key attrs");
            return false;
        }
    ...
    // DECRYPT_KEY (nested)
    {
        ...
        if (!dk)
            return false;
        if (!NlaPut(buf, offset, kAttrsCap, OVPN_KEY_DIR_ATTR_CIPHER_KEY, decrypt_key, cipher_key_size) || !NlaPut(buf, offset, kAttrsCap, OVPN_KEY_DIR_ATTR_NONCE_TAIL, decrypt_nonce_tail, 8))
        {
            logger.error("DCO: Buffer overflow in decrypt key attrs");
            return false;
        }
                        ...
                        spdlog::logger &logger)
{
    ...
    if (!swap_attr)
        return false;
    if (!NlaPut(buf, offset, kAttrsCap, OVPN_SWAP_KEYS_ATTR_PEER_ID, &peer_id, sizeof(peer_id)))
        return false;
                                ...
                                spdlog::logger &logger)
{
    ...
    if (!set_attr)
        return false;
    if (!NlaPut(buf, offset, kAttrsCap, OVPN_SET_PEER_ATTR_PEER_ID, &peer_id, sizeof(peer_id)) || !NlaPut(buf, offset, kAttrsCap, OVPN_SET_PEER_ATTR_KEEPALIVE_INTERVAL, &interval, sizeof(interval)) || !NlaPut(buf, offset, kAttrsCap, OVPN_SET_PEER_ATTR_KEEPALIVE_TIMEOUT, &timeout, sizeof(timeout)))
        return false;
