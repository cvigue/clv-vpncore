...
UdpTransport UdpConnector::Connect(const std::string &host,
                                   std::uint16_t port,
                                   bool dco_mode,
                                   bool ipv6_only)
{
    ...
    // When ipv6_only is requested, supply a protocol hint so the resolver
    // returns only IPv6 addresses (avoids accidentally picking a v4-mapped
    // address that would bypass the intent).
    asio::ip::udp::resolver::results_type endpoints;
    if (ipv6_only)
        endpoints = resolver.resolve(asio::ip::udp::v6(), host, std::to_string(port));
    else
    ...
    if (ipv6_only)
    {
        // IPv6-only mode: derived from proto=udp6 in .ovpn files.
        // Use a pure IPv6 socket with IPV6_V6ONLY set — no v4-mapped addresses.
        socket->open(asio::ip::udp::v6());
        socket->set_option(asio::ip::v6_only(true));
    }
    else if (dco_mode && resolvedEndpoint_.address().is_v4())
    {
