...
class UdpConnector
{
  public:
     ...
     * @param ipv6_only When true, resolve only to IPv6 endpoints and set
     *                  IPV6_V6ONLY on the socket (no dual-stack, no v4-mapped).
     *                  Derived from proto=udp6 in .ovpn files.
     ...
     */
    UdpTransport Connect(const std::string &host,
                         std::uint16_t port,
                         bool dco_mode = false,
...
struct ClientConnector : std::variant<UdpConnector, TcpConnector>
{
     ...
     * @param ipv6_only When true, resolve IPv6-only (UDP only; ignored for TCP)
     * @return TransportHandle wrapping either UdpTransport or TcpTransport
     * @throws std::runtime_error on resolution failure
     * @throws asio::system_error on socket/connection failure
     */
    TransportHandle Connect(const std::string &host,
                            std::uint16_t port,
                            bool dco_mode = false,
                            bool ipv6_only = false)
    {
        return std::visit([&host, port, dco_mode, ipv6_only](auto &connector) -> TransportHandle
        {
