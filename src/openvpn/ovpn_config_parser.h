...
struct ClientConnectionConfig
{
    ...
    /** Optional NCP cipher list from data-ciphers/ncp-ciphers directive */
    std::vector<std::string> data_ciphers;
...
{
  public:
    /** Maximum accepted size of a complete .ovpn config (file or string). */
    static constexpr std::size_t kMaxConfigBytes = 512UL * 1024UL; // 512 KB

    /** Maximum accepted size of a single inline PEM/key block. */
    static constexpr std::size_t kMaxInlineBlockBytes = 64UL * 1024UL; // 64 KB
