...
class DataChannel
{
  public:
    ...
    /**
     * @brief Encrypt a data packet using an externally-supplied packet ID.
     *
     ...
     * @param session_id Session ID for the P_DATA_V2 peer-id field.
     * @param packet_id  Pre-allocated outbound packet ID.
     * @return Serialized encrypted packet, or empty on error.
     */
     ...
     *
     * The caller must place plaintext IP data at buf[kDataV2Overhead..]
    ...
    /**
     * @brief Get the current primary encryption key (read-only).
     *
     * Used by the split-datapath control plane to snapshot encrypt key
     * material for publication via QSBR.
     */
    const EncryptionKey &GetPrimaryEncryptKey() const
    {
        return primary_encrypt_;
    }

    /**
     * @brief Get the current primary decryption key (read-only).
     *
     * Used by the split-datapath control plane to snapshot decrypt key
     * material for publication to the RX thread.
     */
    const EncryptionKey &GetPrimaryDecryptKey() const
    {
        return primary_decrypt_.key;
    }

    /**
     * @brief Install new keys for a key renegotiation
     *
     * Moves the current primary keys to lame duck status and installs
     * new keys as primary. The lame duck key lives until the next rekey
     * overwrites it.
     *
  ...
  private:
    ...
    clv::RateLimiter<> no_key_limiter_;
    clv::RateLimiter<> too_old_limiter_;
