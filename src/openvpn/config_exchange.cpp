...
// dns: compound keyword — `dns server N address <a>` / `dns server N resolve-domains <d...>`
//      or `dns search-domains <d...>`
// args[0] is the first token after "dns", i.e. "server" or "search-domains".
void ApplyDnsOption(NegotiatedConfig &c, const std::vector<std::string> &args)
{
    if (args.empty())
        throw ConfigParseError("dns: missing sub-command");

    if (args[0] == "search-domains")
    {
    ...
    {
        // dns server <priority> <key> <values...>
        if (args.size() < 4)
        ...
        {
            // Find or create the entry for this priority.
            DnsServerEntry *entry = nullptr;
            for (auto &e : c.dns_servers)
        ...
        {
            spdlog::warn("dns server {}: unknown field '{}' — ignored", priority, key);
        }
    }
    else
    {
        spdlog::warn("dns: unknown sub-command '{}' — ignored", args[0]);
    }
}

std::string EmitDnsOptions(const NegotiatedConfig &c)
{
    std::string result;
    for (const auto &entry : c.dns_servers)
    {
        for (const auto &addr : entry.addresses)
        {
            result += ",dns server ";
            result += std::to_string(entry.priority);
            result += " address ";
            result += addr;
        }
        if (!entry.resolve_domains.empty())
        {
            result += ",dns server ";
            result += std::to_string(entry.priority);
            result += " resolve-domains";
            for (const auto &d : entry.resolve_domains)
            {
                result += ' ';
                result += d;
            }
    ...
    if (!c.dns_search_domains.empty())
    {
        result += ",dns search-domains";
        for (const auto &d : c.dns_search_domains)
        {
            result += ' ';
            result += d;
        }
...
std::string EmitRenegSec(const NegotiatedConfig &cfg)
{
    if (cfg.reneg_sec == 0)
        return {};
    return ",reneg-sec " + std::to_string(cfg.reneg_sec);
}
...
static constexpr auto kOptionTable = std::to_array<OptionSpec>({
    ...
    {"dns", ConfigOptionType::DNS_OPTION, ArgMode::ALL, ApplyDnsOption, EmitDnsOptions},
});
...
void ConfigExchange::ProcessPushReply(const std::string &options_str)
{
    if (configured_)
        throw std::logic_error("ProcessPushReply called after already configured");
    ...
    // Validate cipher/auth compatibility (unknown/invalid combinations).
    ...
    // Reject actual compression — we have no compressor/decompressor.
    // Empty or absent "compress" is fine (no framing byte).
    // Mirrors DCO kernel behavior which silently drops compressed data.
    if (!negotiated_config_.compress.empty())
        throw ConfigParseError("compression not supported: " + negotiated_config_.compress);
    ...
    const std::string &peer_info)
{
    ...
    if (!peer_info.empty())
        netcore::append_length_prefixed_string(result, peer_info);

    return result;
}

std::optional<std::tuple<std::vector<std::uint8_t>, std::string, std::string, std::string, std::string>>
ParseKeyMethod2Message(const std::vector<std::uint8_t> &data, bool is_from_server)
{
    ...
    auto peer_info = netcore::read_length_prefixed_string(data_span, pos);
...
std::uint32_t ParseClientIvProto(const std::string &peer_info)
{
    // Scan for a line matching "IV_PROTO=<integer>\n" (or end-of-string)
    constexpr std::string_view kKey = "IV_PROTO=";
    auto pos = peer_info.find(kKey);
