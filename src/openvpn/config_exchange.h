...
/**
 * @brief One server entry from the OpenVPN 2.6+ --dns option.
 *
 * Wire format (server push):
 *   dns server <priority> address <addr[:port]>
 *   dns server <priority> resolve-domains <domain> [...]
 * Multiple `address` lines for the same priority accumulate in `addresses`.
 */
struct DnsServerEntry
{
    int priority = 0;
    std::vector<std::string> addresses;       ///< e.g. {"8.8.8.8", "8.8.4.4"}
    std::vector<std::string> resolve_domains; ///< split-DNS domains for this server
};
...
struct NegotiatedConfig
{
    ...
    /**
     * Structured DNS servers from the OpenVPN 2.6+ `dns server` directive.
     * Populated when the server pushes `dns server N address <addr>` etc.
     * Indexed by insertion order; priority comes from the wire integer.
     */
    std::vector<DnsServerEntry> dns_servers;

    /** Search domains from `dns search-domains <domain> [...]` */
    ...
    /** Renegotiate after N seconds (0 = not pushed / disabled) */
    std::uint32_t reneg_sec = 0;
...
class ConfigExchange
{
  public:
     ...
     * @throws ConfigParseError if any option cannot be parsed or the
     *         resulting configuration is invalid (bad cipher/auth combo,
     *         unsupported compression, etc.).
     * @throws std::logic_error if called after the exchange is already
     *         configured (caller must call Reset() before reuse).
     */
    void ProcessPushReply(const std::string &options_str);
 ...
 * @return tuple of (random_data, options_string, username, password, peer_info) or nullopt on error
 */
...
/**
 * @brief Extract IV_PROTO value from a peer-info string.
 *
 * The peer-info string is a newline-separated sequence of KEY=VALUE lines
 * as sent during key-method 2 (e.g. "IV_VER=1.0\nIV_PROTO=6\n").
 *
 * @return Parsed IV_PROTO bitmask, or 0 if the field is absent / unparseable.
 */
