...
class ControlChannel
{
  public:
     ...
     */
    bool Initialize(PeerRole role, SessionId initial_session_id, std::optional<TlsCertConfig> cert_config);
    ...
    /**
     * @brief Request soft reset (server-initiated renegotiation).
     *
     * Builds P_CONTROL_SOFT_RESET_V1, increments the key ID, reinitialises
     * the TLS context as server (ready to receive the client's ClientHello),
     * and transitions to TlsHandshake.  Must only be called from
     * Active or KeyMaterialReady state.
     *
     * @param cert_config TLS certificate configuration for the new handshake
     * @return Serialized soft reset packet, or empty on error
     */
    std::vector<std::uint8_t> RequestSoftReset(PeerRole role, const TlsCertConfig &cert_config);

    /**
     * @brief Respond to a server-initiated soft reset (client-side renegotiation).
     *
     * Called by the client upon receiving P_CONTROL_SOFT_RESET_V1 from the server.
     * ACKs the incoming packet, increments the key ID, reinitialises TLS as the
     * client role, and returns the client's own P_CONTROL_SOFT_RESET_V1 to echo
     * back to the server (which triggers the server's HandleSoftReset path).
     *
     * @param packet  Incoming soft reset packet from server
     * @param cert_config TLS certificate configuration for the new handshake
     * @return Serialized soft reset packet to send back, or empty on error
     */
    std::vector<std::uint8_t> RespondToSoftReset(const OpenVpnPacket &packet, const TlsCertConfig &cert_config);
    ...
    /**
     * @brief Returns true if any outbound packet has not yet been acknowledged.
     *
     * Used by the server to defer TX-key activation until the client ACKs
     * KEY_METHOD_2, mirroring OpenVPN's reliable_empty() invariant.
     */
