                                  ...
                                  spdlog::logger &logger)
{
    if (tls_crypt)
    {
        auto wrapped = tls_crypt->Wrap(data, role == openvpn::PeerRole::Server);
        if (!wrapped)
        {
                                        ...
                                        spdlog::logger &logger)
{
    ...
    {
        co_await WrapAndSend(tls_crypt, std::move(fragment), role, transport, logger);
    }
    if (!queued.empty())
    ...
    {
        co_await WrapAndSend(tls_crypt, std::move(pkt), role, transport, logger);
                                               ...
                                               bool suppress_ack)
{
    ...
    else if (!suppress_ack)
    {
        ...
        if (!ack.empty())
        {
            co_await WrapAndSend(tls_crypt, std::move(ack), role, transport, logger);
                      ...
                      spdlog::logger &logger)
{
    ...
    const bool is_server = (role == openvpn::PeerRole::Server);
