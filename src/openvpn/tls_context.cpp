...
TlsContext::TlsContext(PeerRole role, std::optional<TlsCertConfig> cert_config, spdlog::logger &logger)
    : ssl_ctx_(role == PeerRole::Server ? TLS_server_method() : TLS_client_method()), logger_(&logger)
{
    ...
    handshake_.emplace(ssl_ctx_, role == PeerRole::Server);
}
