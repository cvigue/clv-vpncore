...
// ---------------------------------------------------------------------------
// ParseIpv4 — not exercised by ParseCidr tests
// ---------------------------------------------------------------------------

TEST(Ipv4UtilsTest, ParseIpv4Valid)
{
    auto r = ipv4::ParseIpv4("10.8.0.1");
    ASSERT_TRUE(r.has_value());
    EXPECT_EQ(*r, 0x0A080001u);
}

TEST(Ipv4UtilsTest, ParseIpv4Zero)
{
    auto r = ipv4::ParseIpv4("0.0.0.0");
    ASSERT_TRUE(r.has_value());
    EXPECT_EQ(*r, 0x00000000u);
}

TEST(Ipv4UtilsTest, ParseIpv4Broadcast)
{
    auto r = ipv4::ParseIpv4("255.255.255.255");
    ASSERT_TRUE(r.has_value());
    EXPECT_EQ(*r, 0xFFFFFFFFu);
}

TEST(Ipv4UtilsTest, ParseIpv4Loopback)
{
    auto r = ipv4::ParseIpv4("127.0.0.1");
    ASSERT_TRUE(r.has_value());
    EXPECT_EQ(*r, 0x7F000001u);
}

TEST(Ipv4UtilsTest, ParseIpv4InvalidOctetAbove255)
{
    EXPECT_FALSE(ipv4::ParseIpv4("256.0.0.1").has_value());
}

TEST(Ipv4UtilsTest, ParseIpv4InvalidTooFewOctets)
{
    EXPECT_FALSE(ipv4::ParseIpv4("10.8.0").has_value());
}

TEST(Ipv4UtilsTest, ParseIpv4InvalidEmpty)
{
    EXPECT_FALSE(ipv4::ParseIpv4("").has_value());
}

TEST(Ipv4UtilsTest, ParseIpv4InvalidAlpha)
{
    EXPECT_FALSE(ipv4::ParseIpv4("abc").has_value());
    EXPECT_FALSE(ipv4::ParseIpv4("10.8.0.x").has_value());
}

// ---------------------------------------------------------------------------
// MaskToPrefix — no tests existed
// ---------------------------------------------------------------------------

TEST(Ipv4UtilsTest, MaskToPrefixClassB)
{
    EXPECT_EQ(ipv4::MaskToPrefix(0xFFFF0000), 16);
}

TEST(Ipv4UtilsTest, MaskToPrefixClassC)
{
    EXPECT_EQ(ipv4::MaskToPrefix(0xFFFFFF00), 24);
}

TEST(Ipv4UtilsTest, MaskToPrefixClassA)
{
    EXPECT_EQ(ipv4::MaskToPrefix(0xFF000000), 8);
}

TEST(Ipv4UtilsTest, MaskToPrefixZero)
{
    EXPECT_EQ(ipv4::MaskToPrefix(0x00000000), 0);
}

TEST(Ipv4UtilsTest, MaskToPrefixAll)
{
    EXPECT_EQ(ipv4::MaskToPrefix(0xFFFFFFFF), 32);
}

TEST(Ipv4UtilsTest, MaskToPrefixRoundTripAllPrefixes)
{
    // CreateMask(n) -> MaskToPrefix should give back n for all valid n
    for (uint8_t n = 0; n <= 32; ++n)
    {
        EXPECT_EQ(ipv4::MaskToPrefix(ipv4::CreateMask(n)), n)
            << "round-trip failed for prefix " << static_cast<int>(n);
    }
}

// ---------------------------------------------------------------------------
// ParseCidr — edge case: non-numeric trailing chars in prefix
// ---------------------------------------------------------------------------

TEST(Ipv4UtilsTest, ParseCidrTrailingCharsInPrefix)
{
    EXPECT_FALSE(ipv4::ParseCidr("10.8.0.0/24abc").has_value());
}
