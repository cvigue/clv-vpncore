...
TEST_F(OvpnConfigParserTest, ParseDataCiphersList)
{
    std::string ovpn_content = R"(
client
remote example.com 1194
...
<ca>CERT</ca>
    ...
    ASSERT_EQ(3u, config.data_ciphers.size());
    EXPECT_EQ("AES-256-GCM", config.data_ciphers[0]);
    EXPECT_EQ("AES-128-GCM", config.data_ciphers[1]);
    EXPECT_EQ("CHACHA20-POLY1305", config.data_ciphers[2]);
}

TEST_F(OvpnConfigParserTest, ParseLegacyNcpCiphersAlias)
{
    std::string ovpn_content = R"(
client
remote example.com 1194
...
<ca>CERT</ca>
    ...
    ASSERT_EQ(2u, config.data_ciphers.size());
    EXPECT_EQ("AES-256-GCM", config.data_ciphers[0]);
    EXPECT_EQ("AES-128-GCM", config.data_ciphers[1]);
}
...
// Size-cap tests (item 7 fix)

TEST_F(OvpnConfigParserTest, RejectFileTooLarge)
{
    // Write a file slightly above kMaxConfigBytes
    fs::path oversized = test_dir / "oversized.ovpn";
    {
        std::ofstream f(oversized);
        // Fill with harmless comment lines to reach the limit
        const std::size_t target = OvpnConfigParser::kMaxConfigBytes + 1;
    ...
    EXPECT_THROW(
        {
            try
            {
                OvpnConfigParser::ParseFile(oversized);
            }
            catch (const std::runtime_error &e)
            {
                EXPECT_TRUE(std::string(e.what()).find("too large") != std::string::npos);
                throw;
            }
        },
        std::runtime_error);
...
TEST_F(OvpnConfigParserTest, RejectStringTooLarge)
{
    // Construct a string slightly above kMaxConfigBytes
    const std::size_t target = OvpnConfigParser::kMaxConfigBytes + 1;
    std::string big_content(target, '#');

    EXPECT_THROW(
        {
            try
            {
                OvpnConfigParser::ParseString(big_content);
            }
            catch (const std::runtime_error &e)
            {
                EXPECT_TRUE(std::string(e.what()).find("too large") != std::string::npos);
...
TEST_F(OvpnConfigParserTest, RejectInlineBlockTooLarge)
{
    // Build a <ca> block whose content exceeds kMaxInlineBlockBytes
    const std::size_t block_payload = OvpnConfigParser::kMaxInlineBlockBytes + 1;
    std::string big_ca = "remote example.com 1194\n<ca>\n";
    big_ca += std::string(block_payload, 'A');
    big_ca += "\n</ca>\n";
    ...
    EXPECT_THROW(
        {
            try
            {
                OvpnConfigParser::ParseString(big_ca);
            }
            catch (const std::runtime_error &e)
            {
                EXPECT_TRUE(std::string(e.what()).find("too large") != std::string::npos);
...
TEST_F(OvpnConfigParserTest, AcceptInlineBlockAtExactLimit)
{
    // A block whose raw byte length (between the tags, before Trim) is exactly
    // kMaxInlineBlockBytes must be accepted (the cap is strictly greater-than).
    // Place the payload directly adjacent to the tags so no newlines inflate the count.
    std::string at_limit = "remote example.com 1194\n<ca>";
    at_limit += std::string(OvpnConfigParser::kMaxInlineBlockBytes, 'A');
    at_limit += "</ca>\n";

    // ParseString runs Validate which requires a CA cert — it just parsed one, so
    // we only care that it does NOT throw a size error. Validate may throw for other
    // reasons (e.g. cert content not real PEM) so we catch specifically size errors.
    try
    {
        OvpnConfigParser::ParseString(at_limit);
    }
    catch (const std::runtime_error &e)
    {
        EXPECT_TRUE(std::string(e.what()).find("too large") == std::string::npos)
            << "Unexpected 'too large' error at exact limit: " << e.what();
    }
}
