...
TEST_F(VpnClientConfigTest, LoadFromFile_ValidConfig)
{
    ...
    EXPECT_EQ(config.client->proto, "udp");
...
TEST_F(VpnClientConfigTest, LoadFromFile_IPv6Server)
{
    ...
    EXPECT_EQ(config.client->proto, "udp");
    EXPECT_FALSE(config.client->ipv6_only);
}

TEST_F(VpnClientConfigTest, LoadFromFile_RejectsUdp6InJson)
{
    std::string json_content = R"({
        "client": {
            "server_host": "2001:db8::1",
    ...
    WriteFile("ipv6_udp6_invalid.json", json_content);

    EXPECT_THROW(
        {
            try
            {
                (void)VpnClientConfig::LoadFromFile((temp_dir_ / "ipv6_udp6_invalid.json").string());
            }
            catch (const std::runtime_error &e)
            {
                EXPECT_NE(std::string(e.what()).find("not valid in JSON config"), std::string::npos);
                throw;
            }
        },
        std::runtime_error);
...
TEST_F(VpnClientTest, EmptyServerHost)
{
    ...
    EXPECT_THROW(VpnClient(io_context_, config_), std::runtime_error);
}

// ============================================================================
// VpnClientConfig::ParseJson / LoadFromOvpnFile / Load  (previously dead)
// ============================================================================

TEST_F(VpnClientConfigTest, ParseJson_ValidJson)
{
    nlohmann::json j = {
        {"client", {{"server_host", "vpn.example.com"}, {"server_port", 1194}, {"proto", "udp"}, {"cipher", "AES-256-GCM"}}}};

    auto config = VpnClientConfig::ParseJson(j);
...
TEST_F(VpnClientConfigTest, LoadFromOvpnFile_ValidFile)
{
    // Uses the real test .ovpn fixture shipped with the test_data directory.
    std::string ovpn_path = std::string(TEST_TMP_DIR) + "/../test_data/test_client.ovpn";

    auto config = VpnClientConfig::LoadFromOvpnFile(ovpn_path);

    // OvpnConfigParser::Validate requires a 'remote' directive
    ASSERT_TRUE(config.client.has_value());
    EXPECT_FALSE(config.client->server_host.empty());
}

TEST_F(VpnClientConfigTest, LoadFromOvpnFile_ParsesDataCiphers)
{
...
CERT
    ...
    WriteFile("data_ciphers_client.ovpn", ovpn_content);

    auto config = VpnClientConfig::LoadFromOvpnFile((temp_dir_ / "data_ciphers_client.ovpn").string());

    ASSERT_TRUE(config.client.has_value());
    ASSERT_EQ(2u, config.client->data_ciphers.size());
    EXPECT_EQ("AES-256-GCM", config.client->data_ciphers[0]);
    EXPECT_EQ("AES-128-GCM", config.client->data_ciphers[1]);
}

TEST_F(VpnClientConfigTest, Load_OvpnExtension_DispatchesToOvpnParser)
{
    // .ovpn extension → LoadFromOvpnFile
    std::string ovpn_path = std::string(TEST_TMP_DIR) + "/../test_data/test_client.ovpn";

    auto config = VpnClientConfig::Load(ovpn_path);

    ASSERT_TRUE(config.client.has_value());
    EXPECT_FALSE(config.client->server_host.empty());
}

TEST_F(VpnClientConfigTest, Load_JsonExtension_DispatchesToJsonParser)
{
    // .json extension → LoadFromFile (JSON parser)
    std::string json_content = R"({
        "client": {
            "server_host": "dispatch-test.example.com",
            "server_port": 443
        }
    })";
    WriteFile("dispatch_test.json", json_content);

    auto config = VpnClientConfig::Load((temp_dir_ / "dispatch_test.json").string());

    ASSERT_TRUE(config.client.has_value());
    EXPECT_EQ(config.client->server_host, "dispatch-test.example.com");
    EXPECT_EQ(config.client->server_port, 443);
}

TEST_F(VpnClientConfigTest, Load_UpperCaseOvpnExtension_DispatchesToOvpnParser)
{
    // .OVPN (uppercase) → also dispatched as ovpn
    std::string ovpn_path = std::string(TEST_TMP_DIR) + "/../test_data/test_client.ovpn";

    // Copy file with uppercase extension
    std::filesystem::path dst = temp_dir_ / "CLIENT.OVPN";
    std::filesystem::copy_file(ovpn_path, dst, std::filesystem::copy_options::overwrite_existing);

    auto config = VpnClientConfig::Load(dst.string());
