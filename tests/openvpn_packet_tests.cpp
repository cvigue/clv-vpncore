...
{
    // Every control opcode must be recognised
    EXPECT_TRUE(IsControlPacket(Opcode::P_CONTROL_HARD_RESET_CLIENT_V1));
    EXPECT_TRUE(IsControlPacket(Opcode::P_CONTROL_HARD_RESET_SERVER_V1));
    EXPECT_TRUE(IsControlPacket(Opcode::P_CONTROL_SOFT_RESET_V1));
...
TEST(OpenVpnPacket, is_data_packet)
{
    ...
    // Control opcodes must NOT be classified as data
...
TEST(OpenVpnPacket, is_hard_reset_classifiers)
{
    // IsHardReset — any hard reset, client or server, any version
    EXPECT_TRUE(IsHardReset(Opcode::P_CONTROL_HARD_RESET_CLIENT_V1));
    EXPECT_TRUE(IsHardReset(Opcode::P_CONTROL_HARD_RESET_SERVER_V1));
    ...
    // IsHardResetClient — only client opcodes
    EXPECT_TRUE(IsHardResetClient(Opcode::P_CONTROL_HARD_RESET_CLIENT_V1));
    EXPECT_TRUE(IsHardResetClient(Opcode::P_CONTROL_HARD_RESET_CLIENT_V2));
...
TEST(OpenVpnPacket, parse_ack_packet_truncated_payload)
{
    // P_ACK_V1: ack_count=3 but only 4 bytes of ACK data present (1 ID instead of 3).
    // Exercises the buffer bounds check in ParsePureAck before iteration.
    std::vector<std::uint8_t> raw_packet = {
        0x28, // Opcode P_ACK_V1, key_id 0
        0x00,
        0x00,
        0x00,
        0x00,
        ...
        0x00,
        0x00,
        0x01, // Only 1 ACK ID (4 bytes), not 3 (12 bytes)
    };
...
TEST(OpenVpnPacket, parse_control_packet_truncated_piggybacked_acks)
{
    // P_CONTROL_V1: piggybacked ack_count=2 but only 4 bytes of ACK data present (1 ID instead of 2).
    // Exercises the buffer bounds check in ParseAckArray before iteration.
    std::vector<std::uint8_t> raw_packet = {
        0x20, // Opcode P_CONTROL_V1, key_id 0
        0x00,
        0x00,
        0x00,
        0x00,
        ...
        0x00,
        0x01, // Session ID
        0x02, // piggybacked ack_count = 2
        0x00,
        0x00,
        0x00,
        0x01, // Only 1 ACK ID (4 bytes), not 2 (8 bytes)
    };
...
// ============================================================================
// Factory method coverage — HardReset v1, HardResetResponse v3, SoftReset, Control
// ============================================================================

TEST(OpenVpnPacket, hard_reset_v1_client)
{
    // version=1 is the V1 legacy opcode path (default case only covers V2/V3 in most tests).
    auto pkt = OpenVpnPacket::HardReset(true, 1, 0, 0xABCDEF0123456789ULL, 42);
    EXPECT_EQ(pkt.opcode_, Opcode::P_CONTROL_HARD_RESET_CLIENT_V1);
    EXPECT_EQ(pkt.key_id_, 0);
    EXPECT_EQ(pkt.session_id_, 0xABCDEF0123456789ULL);
    ASSERT_TRUE(pkt.packet_id_.has_value());
    EXPECT_EQ(pkt.packet_id_.value(), 42u);

    // Must serialize and parse back correctly.
    auto serialized = pkt.Serialize();
    ASSERT_FALSE(serialized.empty());
    auto parsed = OpenVpnPacket::Parse(serialized);
    ASSERT_TRUE(parsed.has_value());
    EXPECT_EQ(parsed->opcode_, Opcode::P_CONTROL_HARD_RESET_CLIENT_V1);
}

TEST(OpenVpnPacket, hard_reset_v1_server)
{
    auto pkt = OpenVpnPacket::HardReset(false, 1, 2, 0x1122334455667788ULL, 7);
    EXPECT_EQ(pkt.opcode_, Opcode::P_CONTROL_HARD_RESET_SERVER_V1);
    EXPECT_EQ(pkt.key_id_, 2);
    ASSERT_TRUE(pkt.packet_id_.has_value());
    EXPECT_EQ(pkt.packet_id_.value(), 7u);
...
{
    auto pkt = OpenVpnPacket::SoftReset(3, 0xFEDCBA9876543210ULL, 99);
    EXPECT_EQ(pkt.opcode_, Opcode::P_CONTROL_SOFT_RESET_V1);
    EXPECT_EQ(pkt.key_id_, 3);
    EXPECT_EQ(pkt.session_id_, 0xFEDCBA9876543210ULL);
    ASSERT_TRUE(pkt.packet_id_.has_value());
    EXPECT_EQ(pkt.packet_id_.value(), 99u);
    EXPECT_TRUE(pkt.payload_.empty()); // Soft reset has no payload
    ...
    EXPECT_EQ(parsed->key_id_, 3);
    ASSERT_TRUE(parsed->packet_id_.has_value());
    EXPECT_EQ(parsed->packet_id_.value(), 99u);
}

TEST(OpenVpnPacket, control_factory)
{
    std::vector<std::uint8_t> tls_data = {0x16, 0x03, 0x03, 0x00, 0x0A, 0x01, 0x02, 0x03, 0x04, 0x05};
    auto pkt = OpenVpnPacket::Control(1, 0x0102030405060708ULL, 5, tls_data);
    EXPECT_EQ(pkt.opcode_, Opcode::P_CONTROL_V1);
    EXPECT_EQ(pkt.key_id_, 1);
    EXPECT_EQ(pkt.session_id_, 0x0102030405060708ULL);
    ASSERT_TRUE(pkt.packet_id_.has_value());
    EXPECT_EQ(pkt.packet_id_.value(), 5u);
    EXPECT_EQ(pkt.payload_, tls_data);
    ...
    auto parsed = OpenVpnPacket::Parse(serialized);
    ASSERT_TRUE(parsed.has_value());
    EXPECT_EQ(parsed->opcode_, Opcode::P_CONTROL_V1);
    EXPECT_EQ(parsed->payload_, tls_data);
}
...
TEST(SessionId, from_bytes_too_short_returns_zero)
{
    // FromBytes with fewer than 8 bytes returns SessionId{0}.
    std::vector<std::uint8_t> short_data = {0x01, 0x02, 0x03};
    auto sid = SessionId::FromBytes(short_data);
    EXPECT_EQ(sid.value, 0u);
}

TEST(SessionId, from_bytes_exactly_eight)
{
    // Exactly 8 bytes should succeed and produce a non-zero session ID.
