...
int main(int argc, char *argv[])
{
    ...
    try
    {
        ...
        // ── Process-wide traffic policy (one zone per simple_vpn instance) ──
        auto &proc_logger = *spdlog::default_logger();
        vpn::TunnelZone tunnel_zone(vpn::BuildZonePolicy(base_config), proc_logger);
        tunnel_zone.EnsureTransitRouting();
        ...
        if (base_config.HasServerRole())
        {
            ...
            server = std::make_unique<vpn::VpnServer>(role->ctx, base_config, tunnel_zone);
        }
