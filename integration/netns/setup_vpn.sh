...
# Clean up any pre-existing namespace (in case of partial failure)
...
ns_exec "${NS_BRIDGE}" ip link set veth-br-srv txqueuelen 10000

# Configure server side
ns_exec "${NS_SERVER}" ip addr add "${SERVER_IP}/${UNDERLAY_CIDR}" dev veth-srv
ns_exec "${NS_SERVER}" ip link set veth-srv up
ns_exec "${NS_SERVER}" ip link set veth-srv txqueuelen 10000
