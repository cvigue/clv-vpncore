#!/bin/bash
# test_it14_mixed_client_dco.sh — IT14: Multi-client mixed DCO/userspace clients
#
# Like IT5 (multi-client DCO) but client-0 runs in userspace mode while
# clients 1 and 2 use DCO. The server runs DCO. This validates that a
# DCO server correctly handles concurrent sessions that use different
# data-path modes.
#
# Validates:
#   - N simultaneous handshakes complete (mixed client modes)
#   - Server creates ovpn-dco0 (DCO MP mode)
#   - Client-0 creates tun0 (userspace fallback)
#   - Clients 1 and 2 create ovpn-clientN (DCO)
...
#
# Usage: sudo ./test_it14_mixed_client_dco.sh [PROJECT_ROOT]
...
# Client-0 runs userspace; clients 1+ run DCO
CLIENT_CONFIGS=(
    "${PROJECT_ROOT}/integration/configs/it_client.json"
    "${PROJECT_ROOT}/integration/configs/it_client_dco.json"
    "${PROJECT_ROOT}/integration/configs/it_client_dco.json"
)
...
echo "=== IT14: Multi-Client Mixed DCO/Userspace (${NUM_CLIENTS} clients, DCO server) ==="

if [[ $(id -u) -ne 0 ]]; then
    exec sudo --preserve-env=PATH "$0" "$@"
fi

# Gate: ovpn-dco kernel module must be available (needed for server + 2 clients)
...
echo "[2/8] Starting ${NUM_CLIENTS} clients (client-0: userspace, clients 1-2: DCO)..."
for i in $(seq 0 $(( NUM_CLIENTS - 1 ))); do
    NS_CLIENT="ns-vpn-client-${i}"
    CLIENT_LOG_I="${LOG_DIR}/client-${i}.log"
    CFG="${CLIENT_CONFIGS[$i]}"

    ns_bg "${NS_CLIENT}" "${BINARY}" "${CFG}" \
        > "${CLIENT_LOG_I}" 2>&1 &
    CLIENT_PIDS+=($!)
    local_mode="DCO"
    (( i == 0 )) && local_mode="userspace"
    echo "      Client-${i} PID: ${CLIENT_PIDS[$i]} (${NS_CLIENT}, ${local_mode})"
done
...
echo "[4/8] Verifying tunnel devices..."

# Server: ovpn-dco0 in MP mode (DCO server)
if ns_exec "${NS_SERVER}" ip link show ovpn-dco0 &>/dev/null; then
    echo "      Server: ovpn-dco0 present (DCO)"
else
    fail "Server DCO device ovpn-dco0 not found — DCO server mode not active"
fi

# Client-0: userspace — expect tun0
if ns_exec "ns-vpn-client-0" ip link show tun0 &>/dev/null; then
    echo "      Client-0: tun0 present (userspace)"
else
    fail "Client-0 tun0 not found — expected userspace TUN device"
fi

# Verify client-0 does NOT have a DCO device (it's userspace)
if ns_exec "ns-vpn-client-0" ip -o link show 2>/dev/null | grep -qP 'ovpn-client\d+'; then
    echo "      Warning: Client-0 unexpectedly has a DCO device (expected userspace TUN)"
fi

# Clients 1+: DCO — expect ovpn-clientN
declare -a DCO_DEVICES=()
DCO_DEVICES+=("")  # placeholder for client-0
for i in 1 2; do
    NS_CLIENT="ns-vpn-client-${i}"
    client_dco_dev=""
    ...
    if [[ -n "${client_dco_dev}" ]]; then
        echo "      Client-${i} (${NS_CLIENT}): ${client_dco_dev} present (DCO)"
        DCO_DEVICES+=("${client_dco_dev}")
    else
        fail "Client-${i} DCO device (ovpn-client*) not found — kernel offload not active"
    fi
done
...
for i in $(seq 0 $(( NUM_CLIENTS - 1 ))); do
    ...
    local_mode="DCO"
    (( i == 0 )) && local_mode="userspace"
    echo "      Client-${i} (${local_mode}): ${TUN_IP}"
done
...
for i in $(seq 0 $(( NUM_CLIENTS - 1 ))); do
    ...
    if ns_exec "${NS_CLIENT}" ping -c "${PING_COUNT}" -W "${PING_TIMEOUT}" \
        ...
        local_mode="DCO"
        (( i == 0 )) && local_mode="userspace"
        echo "      Client-${i} (${local_mode}) → server: OK"
        (( PING_PASS++ )) || true
    else
...
for i in $(seq 0 $(( NUM_CLIENTS - 1 ))); do
    for j in $(seq 0 $(( NUM_CLIENTS - 1 ))); do
        ...
        NS_SRC="ns-vpn-client-${i}"
        DST_IP="${TUNNEL_IPS[$j]}"
        if ns_exec "${NS_SRC}" ping -c 1 -W 2 "${DST_IP}" &>/dev/null; then
            echo "      Client-${i} → Client-${j} (${DST_IP}): still reachable (BAD)"
            neg_ok=0
        else
            echo "      Client-${i} → Client-${j} (${DST_IP}): unreachable (good)"
        fi
    done
done
...
echo ""
echo "=== IT14 PASSED ==="
echo "    DCO server with mixed client modes (1 userspace + 2 DCO)"
echo "    Client-0: userspace TUN data path"
echo "    Clients 1-2: kernel-offloaded DCO data path"
echo "    All ${NUM_CLIENTS} clients got unique tunnel IPs"
