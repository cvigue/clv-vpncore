#!/bin/bash
# test_it15_userspace_server_dco_clients.sh — IT15: Userspace server, DCO clients
#
# Like IT5 (multi-client DCO) but the server runs in userspace mode while
# all 3 clients use DCO. This validates the inverse of IT14: a userspace
# server correctly handles concurrent sessions with DCO clients.
#
# Validates:
#   - N simultaneous handshakes complete (userspace server + DCO clients)
#   - Server creates tun0 (userspace TUN, no ovpn-dco0)
#   - Each client creates an ovpn-clientN DCO device
...
# Prerequisites:
#   - Root / CAP_NET_ADMIN
#   - ovpn-dco kernel module loaded or loadable (needed by DCO clients)
#   - simple_vpn built at build/demos/simple_vpn
#   - Network namespaces set up via integration/netns/setup_vpn.sh 3
#
# Usage: sudo ./test_it15_userspace_server_dco_clients.sh [PROJECT_ROOT]
...
# Server is userspace; clients are DCO
...
echo "=== IT15: Userspace Server + DCO Clients (${NUM_CLIENTS} clients) ==="

if [[ $(id -u) -ne 0 ]]; then
    exec sudo --preserve-env=PATH "$0" "$@"
fi

# Gate: ovpn-dco kernel module must be available (needed for DCO clients)
...
echo "[1/8] Starting userspace server in ${NS_SERVER}..."
...
for i in $(seq 0 $(( NUM_CLIENTS - 1 ))); do
    ...
    ns_bg "${NS_CLIENT}" "${BINARY}" "${CLIENT_CONFIG}" \
        > "${CLIENT_LOG_I}" 2>&1 &
    CLIENT_PIDS+=($!)
    echo "      Client-${i} PID: ${CLIENT_PIDS[$i]} (${NS_CLIENT}, DCO)"
done
...
# Server: must have tun0 (userspace), must NOT have ovpn-dco0
if ns_exec "${NS_SERVER}" ip link show tun0 &>/dev/null; then
    echo "      Server: tun0 present (userspace)"
else
    fail "Server tun0 not found — expected userspace TUN device"
fi

if ns_exec "${NS_SERVER}" ip link show ovpn-dco0 &>/dev/null; then
    fail "Server has ovpn-dco0 — expected userspace mode (no DCO device)"
fi
echo "      Server: no DCO device (correct for userspace mode)"

# Each client: expect ovpn-clientN
...
echo ""
echo "=== IT15 PASSED ==="
echo "    Userspace server with ${NUM_CLIENTS} DCO clients"
echo "    Server: userspace TUN data path (tun0)"
echo "    Clients: kernel-offloaded DCO data path (ovpn-clientN)"
