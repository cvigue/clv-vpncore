...
# Deploy ovpn profile (patch SERVER_IP placeholder if provided)
OVPN="$PROJECT_ROOT/configs/sample_client.ovpn"
if [[ -f "$OVPN" ]]; then
    TMPOVPN=$(mktemp --suffix=.ovpn)
    trap 'rm -f "$TMPCONFIG" "$TMPOVPN"' EXIT
    cp "$OVPN" "$TMPOVPN"
    if [[ -n "$SERVER_IP" ]]; then
        sed -i "s|remote SERVER_IP |remote $SERVER_IP |" "$TMPOVPN"
    fi
    scp -q "$TMPOVPN" "$HOST:$RPATH/client.ovpn"
fi

echo "Done. On $HOST:"
echo "  cd $RPATH && sudo ./simple_vpn client_config.json"
echo "  cd $RPATH && sudo openvpn client.ovpn   # reference client interop test"
